...
void Mediator::RPCListenerThread() {
  ...
  while (status()) {
    ...
    if (connection_socket) {
      ...
      // Store a shared_ptr to the node. TODO: adjust data structures to store key.
...
Json Mediator::jsonUnpublishCallback(const Json &json) {
  return removePublisher(json["topic_name"].get<std::string>(), json["host"].get<std::string>(),
                         json["port"].get<int>(), json["message_name"].get<std::string>());
}

Json Mediator::jsonPublishCallback(const Json &json) {
  return addPublisher(json["topic_name"].get<std::string>(), json["host"].get<std::string>(), json["port"].get<int>(),
                      json["message_name"].get<std::string>());
}

Json Mediator::jsonUnsubscribeCallback(const Json &json) {
  return removeSubscriber(json["topic_name"].get<std::string>(), json["host"].get<std::string>(),
                          json["port"].get<int>(), json["message_name"].get<std::string>());
}

Json Mediator::jsonSubscribeCallback(const Json &json) {
  return addSubscriber(json["topic_name"].get<std::string>(), json["host"].get<std::string>(), json["port"].get<int>(),
                       json["message_name"].get<std::string>());
}

// TODO not sure if these are needed in this way
Json Mediator::addSubscriber(std::string const &topic_name, std::string const &host, int const port,
...
Json Mediator::addPublisher(std::string const &topic_name, std::string const &host, int const port,
                            std::string const &message_name) {
...
Json Mediator::removeSubscriber(std::string const &topic_name, std::string const &host, int const port,
                                std::string const &message_name) {
  ...
  if (it != subscriberTable_.end()) {
    auto jt = std::remove_if(it->second.begin(), it->second.end(),
                             [topic_info](Topic const &topic) -> bool { return topic == topic_info; });
...
Json Mediator::removePublisher(std::string const &topic_name, std::string const &host, int const port,
                               std::string const &message_name) {
