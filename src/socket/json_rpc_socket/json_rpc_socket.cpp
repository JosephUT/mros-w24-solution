...
void JsonRPCSocket::processRequestResponse(const CallbackName& callback_name, nlohmann::json& callback_argument, const CallbackName& response_callback_name) {
  std::lock_guard<std::mutex> lock_guard(request_response_callbacks_lock_);
  ...
  if (request_response_callback_iter != request_response_callbacks_.end()) {
    json callback_result = request_response_callback_iter->second(callback_argument);
