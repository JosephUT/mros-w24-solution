...
void BsonSocket::sendMessage(const json &message) {
  ...
  ssize_t send_size = 0;
  do {
    send_size = send(file_descriptor_, reinterpret_cast<std::uint8_t*>(&bson_size) + send_size, 8 - send_size,
                     MSG_NOSIGNAL);
  ...
  do {
    send_size = send(file_descriptor_, bson.data() + send_size, bson.size() - send_size,
                     MSG_NOSIGNAL);
    size_to_complete_send -= send_size;
    if (send_size == -1) {
      throw SocketErrnoException("Failed to send to peer.");
